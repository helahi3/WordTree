...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode longest = this.getPrefixNode(word);
		String longestString = longest.toString();
		
		String remainder = "";

		//String with all the chars of "word" not contained in longestString
		for(int i=longestString.length(); i < word.length(); i++) remainder += word.charAt(i);
	
		WordTreeNode currNode = longest;
		for(int i=0; i<remainder.length(); i++) {
			char temp = remainder.charAt(i);
			currNode = currNode.createChild(temp);
		}
	...
	WordTreeNode getPrefixNode(String word) 
	{
		...
		return currNode;  //  REPLACE THIS STUB
	...
	{  
		...
		WordTreeNode prefixNode = getPrefixNode(word);
		if(!prefixNode.endOfWord) return false;
		if(word.equals(prefix)) return true;
		
		else if(prefix.length() > 0){
			boolean test = true;
			int i;
			for(i=0; i<word.length(); i++) {
				if(word.charAt(i) != prefix.charAt(i)) test = false;
			}
			return test;
		}
		else return false;
	...
	{
		//  ADD YOUR CODE BELOW 
		ArrayList<String> ListPrefMatches = new ArrayList<String>();

		Stack<WordTreeNode> nodeStack = new Stack<WordTreeNode>();
		WordTreeNode currNode = getPrefixNode(prefix);
		WordTreeNode temp;
		
		if(currNode.toString().length() == 0) return ListPrefMatches;
		
		nodeStack.push(currNode);
		while(!nodeStack.isEmpty()) {

			currNode = (WordTreeNode) nodeStack.pop();
			if(currNode.endOfWord) ListPrefMatches.add(currNode.toString());
			for (int i = 0; i < 256; i++) {
				if (currNode.getChild((char) i) != null) {
					temp = currNode.getChild((char)i);
					nodeStack.push(temp);
				}
			}
			if(nodeStack.size() > 300) break;
		}
		
		return ListPrefMatches;   
	...
	public class WordTreeNode
	{
		...
		{    
			...
			child.parent = this; //IDK IF THIS IS NECESSARY
		...
		public String toString()
		{
			...
			if(charInParent == '\0') return "";			     
			result +=  parent.toString() + this.charInParent; 
			
			
			return result;   //  REMOVE THIS CODE STUB
